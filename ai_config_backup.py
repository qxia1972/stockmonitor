...
- 命令生成最佳实践：创建专用函数生成跨平台兼容的命令，避免硬编码shell特定语法；使用模板字符串和条件逻辑处理不同shell
- AI命令生成强制规则：AI在生成任何shell命令时必须使用run_cross_platform_command()函数或generate_shell_command()函数，确保跨平台兼容性；禁止直接使用&&、||、;等分隔符；必须通过专用函数自动选择正确的分隔符
- 跨平台命令执行标准：所有多命令组合必须使用run_cross_platform_command(*commands)语法，确保在不同shell环境中正确执行；单个命令可以直接执行，但多命令必须使用跨平台函数

文件组织规则：
- test/目录强制使用：所有测试文件必须放在test/目录中，禁止在工作目录创建测试文件；AI必须使用get_test_dir()函数获取test目录路径，确保测试文件路径正确
- 测试文件路径规范：测试脚本必须使用get_test_dir()函数构造文件路径，格式为get_test_dir() / "filename"；禁止使用硬编码的test目录路径
- 测试文件清理强制：所有测试生成的文件必须在测试完成后自动清理，使用os.remove()或shutil.rmtree()；禁止测试文件残留在工作目录
...
- 路径问题：路径权限、长路径限制、空格处理；解决方案：使用pathlib或os.path.join()；Linux长路径无需特殊前缀；测试文件应使用get_test_dir()函数确保在test/目录中操作
...
- 命令分隔符错误：跨平台语法混淆(Bash用&&，PowerShell用;)；解决方案：检测shell类型选择正确分隔符、使用平台特定的命令构建函数、避免硬编码分隔符；AI必须使用run_cross_platform_command()函数自动处理分隔符选择
- 跨平台命令生成规范：AI生成的所有shell命令必须通过专用函数处理；禁止在代码中直接使用&&、||、;等分隔符；必须使用generate_shell_command()或run_cross_platform_command()确保兼容性
- 字符串语法错误强制检查：AI必须在生成代码前验证所有字符串字面量的完整性；禁止未终止的字符串；必须使用原始字符串(r'')处理包含反斜杠的路径；三引号字符串必须正确闭合；字符串嵌套必须使用不同类型的引号
- 路径处理强制规范：所有文件路径必须使用pathlib.Path或os.path.join()构造；AI禁止使用硬编码的反斜杠路径字符串；测试文件路径必须使用get_test_dir()函数；Linux长路径无需特殊前缀
...
- 跨平台命令检查：当生成或执行shell命令时，必须检查是否使用了正确的跨平台函数；如果发现直接使用&&、||、;等分隔符，应立即使用run_cross_platform_command()函数重写
- 命令兼容性验证：生成shell命令后，应验证命令是否能在当前平台上正确执行；优先使用跨平台函数而不是硬编码的命令字符串
- 测试文件路径检查：当创建或操作测试文件时，必须验证是否使用了get_test_dir()函数；如果发现硬编码的test目录路径，应立即重构为get_test_dir() / filename格式
...
def inform_ai_environment_and_rules():
    ...
    print("文件组织规则:")
    print("- test/目录：存放测试文件、中间过程文件、测试文档")
    ...
    print("- 路径问题：路径权限、长路径限制、空格处理；解决方案：使用pathlib或os.path.join()；Linux长路径无需特殊前缀；测试文件应使用get_test_dir()函数确保在test/目录中操作")
