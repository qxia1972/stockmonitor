...
# RQDatac initialization with proper error handling - DELAYED INITIALIZATION
rqdatac_available = False
rqdatac = None
_rqdatac_initialized = False  # 标记是否已初始化

def _ensure_rqdatac_initialized():
    """确保RQDatac已初始化（延迟初始化）"""
    global rqdatac_available, rqdatac, _rqdatac_initialized

    if _rqdatac_initialized:
        return rqdatac_available

    try:
        if rqdatac is None:
            import rqdatac
        # Initialize with license credentials
        username = 'license'
        password = 'Uj6__HTO22yF2JlaNjZoYq-ovfnrdhMMDaktBlIjPaSoAquruZZUpKh4rVmmit0CKYwQZ-Lzz32p8zNdBkQaENUvwZF8oy27_wT0X9k-Sn5OwhHXS0EAwRqmOf1_1em7NZ3FgbyWVBbdBvzz1_nrbjjJ8rqkKd-SeV-JRaWE_G8=VmeTTosP2H0_Qcv5XK1UlUt_6C17yEPmqO0rZb7DVfLeUXOy0rC1pekNVZR5ss1Qh9ER0jk7-NHKHZkAvApr02J4PTe376ywys3usgYBCcxvAoBJAndI-S0OAQQX0B4pPiUr8tPaufTHc_8b7waGitMydTxQCfhY9IqZLv6L9m4='
        addr = ('rqdatad-pro.ricequant.com', 16011)
        rqdatac.init(username=username, password=password, addr=addr)
...
# Use constant for availability check - will be updated after first initialization
RQDATAC_AVAILABLE = False
...
class PoolManager:
    ...
    def __init__(self, data_dir="data", force_refresh_cache: Optional[bool] = None,
                 chunk_days: Optional[int] = None, chunk_retries: Optional[int] = None,
                 chunk_backoff_seconds: Optional[float] = None):
        """
        ...
        # 解析命令行参数 and unified config
        if force_refresh_cache is None:
            # precedence: CLI args override environment/config
            if '--force-refresh' in sys.argv or '-f' in sys.argv:
                self.force_refresh_cache = True
                logger.info("🔄 检测到强制刷新参数（CLI），将从网络获取最新数据")
            else:
                self.force_refresh_cache = bool(FORCE_REFRESH_CACHE or False)
        else:
            self.force_refresh_cache = bool(force_refresh_cache)

        # chunking and retry configuration
        self.chunk_days = int(chunk_days) if chunk_days is not None else int(CHUNK_DAYS)
        self.chunk_retries = int(chunk_retries) if chunk_retries is not None else int(CHUNK_RETRIES)
        self.chunk_backoff_seconds = float(chunk_backoff_seconds) if chunk_backoff_seconds is not None else float(CHUNK_BACKOFF_SECONDS)
    ...
    def _batch_fetch_valuation_data(self, stock_codes: List[str], target_date: str,
        ...
        try:
            # 确保RQDatac已初始化
            if not _ensure_rqdatac_initialized() or rqdatac is None:
    ...
    def sync_and_build_pools_optimized(self, force: bool = False) -> bool:
        """运行每日同步、计算和构建股票池的完整流程 - 优化版本，减少数据拷贝

        Args:
            force: 如果为True则忽略每日仅运行一次的限制，强制执行同步
        """
        # 防止重复并发执行
        if getattr(self, 'is_syncing', False):
            self.logger.info("⏱️ 同步已经在进行中，跳过重复触发")
            return True

        self.is_syncing = True
        try:
            # 检查是否已经为当前交易日运行过（非强制模式下跳过）
            try:
                current_td = None
                if hasattr(self, 'data_store') and hasattr(self.data_store, 'get_target_trading_date'):
                    current_td = self.data_store.get_target_trading_date()
                last_td = getattr(self, 'last_sync_trading_date', None)
                if (not force) and (not getattr(self, 'force_refresh_cache', False)) and (current_td is not None) and (last_td == current_td):
                    self.logger.info(f"📅 今日({current_td})已完成同步，跳过重复执行")
                    return True
            except Exception:
                # 如果检查失败，继续尝试同步
                pass
            ...
            try:
                ...
                if hasattr(self, 'data_store') and hasattr(self.data_store, 'get_target_trading_date'):
                    try:
                        self.last_sync_trading_date = self.data_store.get_target_trading_date()
                    except Exception:
                        pass
            except Exception:
                pass
...
class StockPoolDataStore:
    ...
    def get_price(self, stock_code: str, start_date: str, end_date: str, frequency: str = '1d') -> Optional[pd.DataFrame]:
        """
        获取单只股票的价格数据
        ...
        try:
            ...
            # 尝试使用 rqdatac（支持 daily/分钟 等 frequency）
            price_data = None
            if _ensure_rqdatac_initialized() and rqdatac is not None:
                try:
                    # If requesting minute frequency, do chunked fetch to avoid very large single requests
                    if isinstance(frequency, str) and frequency.endswith('m'):
                        # chunked fetch using configured chunk size and retries
                        s_date = pd.to_datetime(start_date).date()
                        e_date = pd.to_datetime(end_date).date()
                        parts = []
                        cur_start = s_date
                        while cur_start <= e_date:
                            cur_end = min(e_date, cur_start + timedelta(days=self.chunk_days - 1))
                            retries_left = int(self.chunk_retries)
                            got = None
                            while retries_left >= 0:
                                try:
                                    got = rqdatac.get_price(
                                        stock_code,
                                    ...
                                    retries_left -= 1
                                    time.sleep(self.chunk_backoff_seconds)
                            if got is not None and not got.empty:
                                parts.append(got)
                            cur_start = cur_end + timedelta(days=1)
                        if parts:
                            price_data = pd.concat(parts)
                        else:
                            price_data = None
                    else:
                    ...
                    logger.debug(f"rqdatac.get_price failed: {e}")

            # 如果 rqdatac 未返回（或不可用），尝试从本地 data_store 获取（按日线）
            if price_data is None or (hasattr(price_data, 'empty') and price_data.empty):
                # If force_refresh_cache is enabled, do NOT fall back to local cache
                if getattr(self, 'force_refresh_cache', False):
                    logger.info(f"🔄 force_refresh_cache=True: 跳过本地缓存，直接返回空或远端结果 for {stock_code}")
                else:
                    try:
                        # data_store 可能只支持日线/已缓存数据
                        if hasattr(self.data_store, 'get_price'):
                            df_local = self.data_store.get_price(stock_code, start_date=start_date, end_date=end_date)
            ...
            if price_data is None or (hasattr(price_data, 'empty') and price_data.empty):
                logger.warning(f"⚠️ 未获取到{stock_code}的价格数据 (frequency={frequency})")
                return None

            # normalize: if 'date' in columns use it as index; else try to leave index as-is
    ...
    def _fetch_price_series(self, stock_code: str, start_date: str, end_date: str, force_refresh: Optional[bool] = None) -> Optional[pd.DataFrame]:
        """
        获取K线数据 (优化后的缓存逻辑)
        ...
        # Determine effective force_refresh: explicit arg > manager setting
        if force_refresh is None:
            force_refresh_effective = bool(getattr(self, 'force_refresh_cache', False))
        else:
            force_refresh_effective = bool(force_refresh)

        # 如果不是强制刷新，先尝试从统一缓存加载
        if not force_refresh_effective:
...
class StockPoolGUI:
    ...
    集成stockmonitor的显示功能，提供完整的股票池管理界面
    """
    ...
    def __init__(self, pool_manager: PoolManager):
        ...
        Args:
            pool_manager: 股票池管理器实例
        """
        self.pool_manager = pool_manager
        self.logger = pool_manager.logger

        # GUI组件
        self.root: Optional[tk.Tk] = None
        # 左侧改用分区Treeview（父节点表示池分区）
        self.pool_tree: Optional[ttk.Treeview] = None
        self.fig: Optional[Figure] = None
        self.canvas: Optional[FigureCanvasTkAgg] = None
        self.event_tree: Optional[ttk.Treeview] = None
        self.stock_event_tree: Optional[ttk.Treeview] = None
        self.status_label: Optional[tk.Label] = None

        # 控制变量
        self.current_stock_code: Optional[str] = None
        self.current_pool_type = "core"
        self.timeframe_var: Optional[tk.StringVar] = None
        self.style_var: Optional[tk.StringVar] = None
        self.indicator_vars: Dict[str, tk.BooleanVar] = {}
        self.analysis_vars: Dict[str, tk.BooleanVar] = {}

        # 回调函数
        self.stock_selection_callback: Optional[Callable] = None
        self.pool_change_callback: Optional[Callable] = None
    ...
    def _setup_ui(self):
        """设置用户界面"""
        # Configure matplotlib fonts for CJK support (fallback order)
        try:
            import matplotlib
            matplotlib.rcParams['font.sans-serif'] = [
                'SimHei', 'Arial Unicode MS', 'Noto Sans CJK SC', 'Noto Sans CJK JP', 'DejaVu Sans'
            ]
    ...
    def _setup_left_panel(self):
        ...
        # 标题
        tk.Label(left_frame, text="股票池管理", font=('SimHei', 12, 'bold'), bg='lightgray').pack(pady=5)

        # Treeview 分区容器
        tree_frame = tk.Frame(left_frame, bg='lightgray')
        tree_frame.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)

        self.pool_tree = ttk.Treeview(tree_frame, show='tree')
        self.pool_tree.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        tree_scroll = ttk.Scrollbar(tree_frame, orient=tk.VERTICAL, command=self.pool_tree.yview)
        self.pool_tree.configure(yscrollcommand=tree_scroll.set)
        tree_scroll.pack(side=tk.RIGHT, fill=tk.Y)

        # 插入父节点
        self.pool_tree.insert('', 'end', 'pool_core', text='核心池')
        self.pool_tree.insert('', 'end', 'pool_watch', text='观察池')
        self.pool_tree.insert('', 'end', 'pool_basic', text='基础池')

        self.pool_tree.bind('<<TreeviewSelect>>', self._on_pool_tree_selection)

        # 延迟填充
        self._populate_pool_sections()

        # 列表区域
        list_frame = tk.Frame(left_frame, bg='lightgray')
        list_frame.pack(fill=tk.BOTH, expand=False, padx=5, pady=5)

        self.listbox = tk.Listbox(list_frame, height=8)
        self.listbox.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        lb_scroll = ttk.Scrollbar(list_frame, orient=tk.VERTICAL, command=self.listbox.yview)
        self.listbox.configure(yscrollcommand=lb_scroll.set)
        lb_scroll.pack(side=tk.RIGHT, fill=tk.Y)

        self.listbox.bind('<<ListboxSelect>>', self._on_stock_selection)

        # 操作按钮区域
        button_frame = tk.Frame(left_frame, bg='lightgray')
        button_frame.pack(fill=tk.X, padx=5, pady=5)

        tk.Button(button_frame, text="刷新数据", command=self._refresh_data,
                  font=('SimHei', 9)).pack(fill=tk.X, pady=2)

        tk.Button(button_frame, text="同步缓存", command=self._sync_cache,
                  font=('SimHei', 9)).pack(fill=tk.X, pady=2)

        self.force_sync_button = tk.Button(button_frame, text="强制同步", command=self._force_sync,
                                           fg='white', bg='red', font=('SimHei', 9, 'bold'))
        self.force_sync_button.pack(fill=tk.X, pady=2)
    ...
    def _setup_center_panel(self):
        ...
        # 时间周期选择器
        self._setup_timeframe_selector(center_frame)
        ...
        # 图表控制面板
        self._setup_chart_controls(center_frame)
    
    def _setup_timeframe_selector(self, parent):
        """设置时间周期选择器"""
        selector_frame = tk.Frame(parent, bg='white')
        selector_frame.pack(fill=tk.X, padx=5, pady=5)
        
        tk.Label(selector_frame, text="时间周期:", font=('SimHei', 10, 'bold'), bg='white').pack(side=tk.LEFT)
        
        self.timeframe_var = tk.StringVar(value='60min')
        timeframes = [
            ('1分钟', '1min'), ('5分钟', '5min'), ('15分钟', '15min'),
            ('30分钟', '30min'), ('60分钟', '60min'), ('日线', 'daily'),
            ('周线', 'weekly'), ('月线', 'monthly')
        ]
        
        for text, value in timeframes:
            rb = tk.Radiobutton(selector_frame, text=text, variable=self.timeframe_var, 
                              value=value, bg='white', command=self._on_timeframe_changed)
            rb.pack(side=tk.LEFT, padx=5)
    
    def _setup_chart_controls(self, parent):
        """设置图表控制面板"""
        controls_frame = tk.Frame(parent, bg='white')
        controls_frame.pack(fill=tk.X, padx=5, pady=5)
        
        # 技术指标选择（右键菜单）
        indicator_frame = tk.Frame(controls_frame, bg='white')
        indicator_frame.pack(side=tk.LEFT)
        tk.Label(indicator_frame, text="技术指标 (右键选择)", font=('SimHei', 10, 'bold'), bg='white').pack(side=tk.LEFT)

        # 构建右键菜单
        self.indicator_menu = tk.Menu(self.root, tearoff=0)
        indicators = ['MA', 'RSI', 'MACD', 'BOLL', 'VOL']
        for indicator in indicators:
            var = tk.BooleanVar(value=(indicator in ['MA', 'VOL']))
            self.indicator_vars[indicator] = var
            # 使用lambda默认参数避免闭包问题
            self.indicator_menu.add_checkbutton(label=indicator, variable=var, command=lambda ind=indicator: self._toggle_indicator(ind))

        # 右键弹出菜单绑定
        indicator_frame.bind('<Button-3>', lambda e: self.indicator_menu.tk_popup(e.x_root, e.y_root))
        
        # 分析工具选择
        analysis_frame = tk.Frame(controls_frame, bg='white')
        analysis_frame.pack(side=tk.LEFT, padx=20)
        
        tk.Label(analysis_frame, text="分析工具:", font=('SimHei', 10, 'bold'), bg='white').pack(side=tk.LEFT)
        
        analysis_tools = ['趋势线', '支撑线', '阻力线', '斐波那契']
        for tool in analysis_tools:
            var = tk.BooleanVar(value=False)
            self.analysis_vars[tool] = var
            cb = tk.Checkbutton(analysis_frame, text=tool, variable=var, 
                              bg='white', command=self._on_analysis_tools_changed)
            cb.pack(side=tk.LEFT, padx=2)
        
        # 图表样式选择
        style_frame = tk.Frame(controls_frame, bg='white')
        style_frame.pack(side=tk.RIGHT)
        
        tk.Label(style_frame, text="样式:", font=('SimHei', 10, 'bold'), bg='white').pack(side=tk.LEFT)
        
        self.style_var = tk.StringVar(value='classic')
        styles = [('经典', 'classic'), ('现代', 'modern'), ('暗色', 'dark'), ('极简', 'minimal')]
        
        for text, value in styles:
            rb = tk.Radiobutton(style_frame, text=text, variable=self.style_var, 
                              value=value, bg='white', command=self._on_style_changed)
            rb.pack(side=tk.LEFT, padx=5)
    
    def _setup_right_panel(self):
        """设置右侧面板 - 事件和信息"""
        right_frame = tk.Frame(self.root, width=600, bg='lightblue')
        right_frame.pack(side=tk.LEFT, fill=tk.Y)
        right_frame.pack_propagate(False)
        
        # 股票池统计信息
        tk.Label(right_frame, text="股票池统计", font=('SimHei', 11, 'bold'), bg='lightblue').pack(pady=5)
        
        stats_frame = tk.Frame(right_frame, bg='lightblue')
        stats_frame.pack(fill=tk.X, padx=5, pady=5)
        
        self.stats_labels = {}
        stats_items = ['总股票数', '平均评分', '更新时间']
        for item in stats_items:
            frame = tk.Frame(stats_frame, bg='lightblue')
            frame.pack(fill=tk.X, pady=2)
            tk.Label(frame, text=f"{item}:", font=('SimHei', 9), bg='lightblue').pack(side=tk.LEFT)
            self.stats_labels[item] = tk.Label(frame, text="--", font=('SimHei', 9, 'bold'), 
                                             bg='lightblue', fg='blue')
            self.stats_labels[item].pack(side=tk.RIGHT)
        # 股票池事件区域（占位）
        tk.Label(right_frame, text="股票池事件", font=('SimHei', 11, 'bold'), bg='lightblue').pack(pady=(10,5))

        event_tree_frame = tk.Frame(right_frame)
        event_tree_frame.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)

        # 这里暂时仅显示占位文本，后续通过 show_events 填充
        placeholder = tk.Label(event_tree_frame, text='暂无事件（占位）', bg='white', fg='gray', font=('SimHei', 10), relief=tk.GROOVE)
        placeholder.pack(fill=tk.BOTH, expand=True)
        self.event_placeholder = placeholder

        # 当前股票详情区域
        tk.Label(right_frame, text="当前股票详情", font=('SimHei', 11, 'bold'), bg='lightblue').pack(pady=(10,5))

        stock_detail_frame = tk.Frame(right_frame, bg='lightblue')
        stock_detail_frame.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)

        self.stock_detail_text = tk.Text(stock_detail_frame, height=8, font=('SimHei', 9), 
                                       bg='white', wrap=tk.WORD)
        detail_scrollbar = ttk.Scrollbar(stock_detail_frame, orient=tk.VERTICAL, 
                                        command=self.stock_detail_text.yview)
        self.stock_detail_text.configure(yscrollcommand=detail_scrollbar.set)
        self.stock_detail_text.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        detail_scrollbar.pack(side=tk.RIGHT, fill=tk.Y)

    def _populate_pool_sections(self):
        """Populate the treeview with pools from pool_manager."""
        try:
            # Clear existing children
            for parent in ('pool_core', 'pool_watch', 'pool_basic'):
                for child in self.pool_tree.get_children(parent):
                    self.pool_tree.delete(child)

            pools = {}
            if hasattr(self.pool_manager, 'get_all_pools'):
                pools = self.pool_manager.get_all_pools() or {}

            mapping = {
                'core_pool': 'pool_core',
                'watch_pool': 'pool_watch',
                'basic_pool': 'pool_basic',
                'core': 'pool_core',
                'watch': 'pool_watch',
                'basic': 'pool_basic'
            }

            # If all pools are empty or missing, trigger a background sync to populate them
            def _pools_have_data(pools_dict):
                try:
                    for k in ('basic_pool', 'watch_pool', 'core_pool'):
                        v = pools_dict.get(k)
                        if v is None:
                            continue
            ...
            if not _pools_have_data(pools):
                # Insert placeholder children to indicate loading
                for parent_id, title in (('pool_core', '加载中...'), ('pool_watch', '加载中...'), ('pool_basic', '加载中...')):
                    # remove any existing placeholders first
                    for ch in self.pool_tree.get_children(parent_id):
                        self.pool_tree.delete(ch)
                    self.pool_tree.insert(parent_id, 'end', iid=f"{parent_id}_loading", text=title)

                # Run a background sync to populate pools, then refresh UI
                import threading

                # Do not start another background sync if one is already running or finished recently
                try:
                    syncing = getattr(self.pool_manager, 'is_syncing', False)
                    last_sync = getattr(self.pool_manager, 'last_sync_time', None)
                    last_sync_td = getattr(self.pool_manager, 'last_sync_trading_date', None)
                    # If manager is syncing, skip
                    if syncing:
                        self.logger.debug("Background sync already running, skipping start")
                        return
                    # If we've already synced for today's trading date, skip
                    try:
                        current_td = None
                        if hasattr(self.pool_manager, 'data_store') and hasattr(self.pool_manager.data_store, 'get_target_trading_date'):
                            current_td = self.pool_manager.data_store.get_target_trading_date()
                        if current_td is not None and last_sync_td == current_td:
                            self.logger.debug(f"Background: already synced for trading date {current_td}, skipping")
                            return
                    except Exception:
                        # fallback to short-time guard
                        try:
                            if last_sync is not None and (datetime.now() - last_sync).total_seconds() < 60:
                                self.logger.debug("Background sync ran recently, skipping restart")
                                return
                        except Exception:
                            pass
                except Exception:
                ...
                def _bg_sync_and_refresh():
                    try:
                        if hasattr(self.pool_manager, 'sync_and_build_pools_optimized'):
                            self.pool_manager.sync_and_build_pools_optimized()
                    except Exception as e:
                        self.logger.debug(f"Background sync failed: {e}")
                    finally:
                        # schedule UI refresh on main thread
                        try:
                            self.root.after(100, self._populate_pool_sections)
                        except Exception:
                            pass

                threading.Thread(target=_bg_sync_and_refresh, daemon=True).start()
                return

            if isinstance(pools, dict):
                for key, parent_id in mapping.items():
                    if key not in pools:
                        continue
                    pool_obj = pools.get(key)
                    # pool_obj may be a list of codes, or a DataFrame with index or 'code' column
                    codes = []
                    if isinstance(pool_obj, list):
                        codes = pool_obj
                    elif isinstance(pool_obj, pd.DataFrame):
                        # try index or 'code' column
                        if pool_obj.index.name:
                            codes = list(map(str, pool_obj.index.tolist()))
                        elif 'code' in pool_obj.columns:
                            codes = pool_obj['code'].astype(str).tolist()
                        else:
                            # fallback: use first column as code
                            codes = pool_obj.iloc[:, 0].astype(str).tolist()
                    else:
                        # try to iterate
                        try:
                            codes = list(pool_obj)
                    ...
                    for stock in codes:
                        code = stock if isinstance(stock, str) else str(stock)
                        # ensure unique iid
                        iid = f"{parent_id}_{code}"
                        self.pool_tree.insert(parent_id, 'end', iid=iid, text=code)
        except Exception as e:
            self.logger.debug(f"_populate_pool_sections failed: {e}")

    def _on_pool_tree_selection(self, event):
        """Handle selection of pool tree items."""
        sel = self.pool_tree.selection()
        if not sel:
            return
        item = sel[0]
        if item in ('pool_core', 'pool_watch', 'pool_basic'):
            # Notify pool change
            if self.pool_change_callback:
                # map ids back to keys
                mapping = {'pool_core': 'core', 'pool_watch': 'watch', 'pool_basic': 'basic'}
                self.pool_change_callback(mapping.get(item, 'core'))
            # load pool into listbox
            mapping = {'pool_core': 'core_pool', 'pool_watch': 'watch_pool', 'pool_basic': 'basic_pool'}
            pool_key = mapping.get(item, 'core_pool')
            self._load_pool_into_list(pool_key)
            # expand the node
            self.pool_tree.item(item, open=True)
            return
        # else it's a stock code
        # item is an iid like 'pool_core_CODE' - extract code
        if '_' in item:
            code = item.split('_', 1)[1]
        else:
            code = item
        # set current stock and update chart
        self.current_stock_code = code
        try:
            self._update_chart(code)
        except Exception:
            pass
        # external callback
        if self.stock_selection_callback:
            self.stock_selection_callback(code)

    def _load_pool_into_list(self, pool_key: str):
        """Load stocks from a pool into the left listbox."""
        try:
            # Try to obtain the raw pool object from PoolManager
            if hasattr(self.pool_manager, 'get_all_pools'):
                all_pools = self.pool_manager.get_all_pools() or {}
                pool_obj = all_pools.get(pool_key)
            else:
                pool_obj = None

            # Extract a simple list of stock codes from variety of pool representations
            codes = []
            if pool_obj is None:
                codes = []
            elif isinstance(pool_obj, list):
                codes = list(map(str, pool_obj))
            elif isinstance(pool_obj, pd.DataFrame):
                # prefer an explicit 'stock_code' or 'code' column, then index, then first column
                if 'stock_code' in pool_obj.columns:
                    codes = pool_obj['stock_code'].dropna().astype(str).tolist()
                elif 'code' in pool_obj.columns:
                    codes = pool_obj['code'].dropna().astype(str).tolist()
                elif pool_obj.index.name:
                    codes = list(map(str, pool_obj.index.tolist()))
                else:
                    try:
                        codes = pool_obj.iloc[:, 0].dropna().astype(str).tolist()
                    except Exception:
                        codes = []
            else:
                try:
                    codes = list(map(str, list(pool_obj)))
                except Exception:
                    codes = []

            # Helper to extract name and score metadata for a single code
            def _get_meta_for_code(code: str):
                name = None
                score = None

                # If pool_obj is a DataFrame, try to extract a row for this code
                try:
                    if isinstance(pool_obj, pd.DataFrame):
                        row = None
                        if 'stock_code' in pool_obj.columns:
                            mask = pool_obj['stock_code'].astype(str) == str(code)
                            if mask.any():
                                row = pool_obj.loc[mask].iloc[0]
                        elif 'code' in pool_obj.columns:
                            mask = pool_obj['code'].astype(str) == str(code)
                            if mask.any():
                                row = pool_obj.loc[mask].iloc[0]
                        elif pool_obj.index.name:
                            try:
                                row = pool_obj.loc[str(code)]
                            except Exception:
                                # index lookup may fail; fall back
                                pass

                        if row is not None:
                            # name field heuristics
                            for ncol in ('name', 'stock_name', 'symbol', 'sec_name'):
                                if ncol in row.index and pd.notna(row.get(ncol)):
                                    name = str(row.get(ncol))
                                    break

                            # score heuristics
                            for scol in ('score', 'basic_score', 'watch_score', 'core_score', 'COMBINED_SCORE'):
                ...
                # If still missing, try to ask PoolManager for a richer record
                if (name is None or score is None) and hasattr(self.pool_manager, 'find_stock_in_pools_by_code'):
                    try:
                        df = self.pool_manager.find_stock_in_pools_by_code(str(code))
            ...
            # populate listbox with richer display strings
            if self.listbox:
                self.listbox.delete(0, tk.END)
                for code in codes:
                    try:
                        name, score = _get_meta_for_code(code)
                        parts = [str(code)]
                        if name:
                        ...
                        if score is not None:
                            try:
                                display = f"{display} ({float(score):.1f})"
                            except Exception:
                                display = f"{display} ({score})"
                        self.listbox.insert(tk.END, display)
                    except Exception:
                        # fallback to code only when anything goes wrong
                        try:
                            self.listbox.insert(tk.END, str(code))
                        except Exception:
                            pass

        except Exception as e:
            self.logger.debug(f"_load_pool_into_list failed: {e}")

    def _toggle_indicator(self, indicator):
        var = self.indicator_vars.get(indicator)
        if var is None:
            return
        var.set(not var.get())
        # refresh chart if current stock set
        if self.current_stock_code:
            try:
                self._update_chart(self.current_stock_code)
            except Exception:
                pass
    ...
    def _on_stock_selection(self, event):
        ...
        if selection and self.stock_selection_callback:
            index = selection[0]
            stock_text = self.listbox.get(index)
            stock_code = stock_text.split(' ')[0]
            self.stock_selection_callback(stock_code)
    
    def _on_pool_change(self, pool_type: str):
        """池类型改变事件处理"""
        self.current_pool_type = pool_type
        if self.pool_change_callback:
            self.pool_change_callback(pool_type)
    ...
    def _on_indicators_changed(self):
        """技术指标改变处理"""
        if self.current_stock_code:
            self._update_chart(self.current_stock_code)
    
    def _on_analysis_tools_changed(self):
        """分析工具改变处理"""
        if self.current_stock_code:
            self._update_chart(self.current_stock_code)
    ...
    def _refresh_data(self):
        """刷新数据"""
        try:
            self._show_status("正在刷新数据...")
            # 这里可以添加数据刷新逻辑
            self._show_status("数据刷新完成")
        except Exception as e:
            self.logger.error(f"数据刷新失败: {e}")
            self._show_status(f"数据刷新失败: {e}")
    
    def _sync_cache(self):
        """同步缓存"""
        try:
            self._show_status("正在同步缓存...")
            # 这里可以添加缓存同步逻辑
            self._show_status("缓存同步完成")
        except Exception as e:
            self.logger.error(f"缓存同步失败: {e}")
            self._show_status(f"缓存同步失败: {e}")

    def _force_sync(self):
        """强制同步（在后台运行，force=True）"""
        try:
            # disable button to prevent repeated clicks
            try:
                if hasattr(self, 'force_sync_button') and self.force_sync_button is not None:
                    self.force_sync_button.config(state=tk.DISABLED)
            except Exception:
                pass

            self._show_status("正在强制同步，请稍候...")

            def _run():
                try:
                    if hasattr(self.pool_manager, 'sync_and_build_pools_optimized'):
                        self.pool_manager.sync_and_build_pools_optimized(force=True)
                except Exception as e:
                    ...
                    try:
                        self.root.after(100, self._populate_pool_sections)
                        self.root.after(100, lambda: self._show_status('强制同步完成'))
                    except Exception:
                        pass
                    try:
                        if hasattr(self, 'force_sync_button') and self.force_sync_button is not None:
                            self.force_sync_button.config(state=tk.NORMAL)
                    except Exception:
                        pass
            ...
            self.logger.error(f"_force_sync 调用失败: {e}")
            try:
                if hasattr(self, 'force_sync_button') and self.force_sync_button is not None:
                    self.force_sync_button.config(state=tk.NORMAL)
            except Exception:
    ...
    def _update_chart(self, stock_code: str):
        """更新图表"""
        try:
            ...
            # 获取K线数据
            timeframe = self.timeframe_var.get() if self.timeframe_var else '60min'
            kline_data = self._get_kline_data(stock_code, timeframe)
            
            if kline_data is not None and not kline_data.empty:
                self._draw_kline_chart(stock_code, kline_data)
    ...
    def _get_kline_data(self, stock_code: str, timeframe: str) -> Optional[pd.DataFrame]:
        """获取K线数据"""
        try:
            ...
            # Determine frequency param for PoolManager.get_price
            if tf.endswith('min'):
                minutes = ''.join([c for c in tf if c.isdigit()])
                freq = f"{minutes}m" if minutes else '60m'
            elif tf in ('daily', 'd', '1d'):
                freq = '1d'
            else:
                # default to daily
                freq = '1d'
            ...
            df = self.pool_manager.get_price(stock_code, start_str, end_str, frequency=freq)
            return df
        except Exception as e:
            self.logger.error(f"获取K线数据失败: {e}")
            return None
    
    def _draw_kline_chart(self, stock_code: str, kline_data: pd.DataFrame):
        """绘制K线图"""
        try:
            if kline_data.empty:
                return
            ...
            for i, (_, row) in enumerate(kline_data.iterrows()):
                ...
                # 绘制影线
                ax.vlines(i, row['low'], row['high'], colors=color, linewidth=1, alpha=0.8)
            ...
            ax.set_title(f'{stock_code} K线图', fontsize=14, fontweight='bold')
            ...
            # 绘制技术指标
            # MA: 5/10/20
            if self.indicator_vars.get('MA') and self.indicator_vars['MA'].get():
                for period, color in ((5, 'orange'), (10, 'purple'), (20, 'brown')):
                    ma = kline_data['close'].rolling(period).mean()
                    ax.plot(range(len(ma)), ma, label=f'MA{period}', color=color, linewidth=1.0, alpha=0.9)

            # BOLL
            if self.indicator_vars.get('BOLL') and self.indicator_vars['BOLL'].get():
                m = kline_data['close'].rolling(20).mean()
                sd = kline_data['close'].rolling(20).std()
                upper = m + 2 * sd
                lower = m - 2 * sd
                ax.plot(range(len(upper)), upper, linestyle='--', color='gray', linewidth=0.8, alpha=0.8)
                ax.plot(range(len(lower)), lower, linestyle='--', color='gray', linewidth=0.8, alpha=0.8)

            # VOL: 绘制在主图底部缩略
            if self.indicator_vars.get('VOL') and self.indicator_vars['VOL'].get():
                try:
                    ax_vol = self.fig.add_axes([0.125, 0.15, 0.775, 0.2], sharex=ax)
                    vols = kline_data['volume']
                    ax_vol.bar(range(len(vols)), vols, color='gray', alpha=0.6)
    ...
    def show_stock_list(self, stocks: Dict[str, Any]):
        """显示股票列表"""
        if not self.listbox:
            return
        ...
        for stock_code, stock_info in stocks.items():
            if isinstance(stock_info, dict):
                ...
                self.listbox.insert(tk.END, f"{stock_code} {name} ({score:.1f})")
            else:
                self.listbox.insert(tk.END, f"{stock_code} ({stock_info})")
    
    def update_chart(self, stock_code: str, kline_data: Any, volatility_data: Optional[Any] = None):
        """更新图表"""
        self.current_stock_code = stock_code
        self._update_chart(stock_code)
    
    def show_events(self, events: List[Dict[str, Any]]):
        """显示事件"""
    ...
    def show_stock_detail(self, stock_code: str, details: Dict[str, Any]):
        """显示股票详情"""
        ...
        detail_text = f"股票代码: {stock_code}\n"
        detail_text += f"股票名称: {details.get('name', 'N/A')}\n"
        detail_text += f"当前价格: {details.get('current_price', 'N/A')}\n"
        detail_text += f"市值: {details.get('market_cap', 'N/A')}\n"
        detail_text += f"评分: {details.get('score', 'N/A')}\n"
        detail_text += f"PE: {details.get('pe', 'N/A')}\n"
        detail_text += f"PB: {details.get('pb', 'N/A')}\n"
        
        self.stock_detail_text.insert(tk.END, detail_text)
    ...
    def set_stock_selection_callback(self, callback: Callable):
        """设置股票选择回调"""
        self.stock_selection_callback = callback
    
    def set_pool_change_callback(self, callback: Callable):
        """设置池改变回调"""
        self.pool_change_callback = callback
...
def launch_gui(pool_manager: PoolManager):
    """
    启动GUI界面
    ...
    """
    try:
        logger.info("🚀 启动股票池GUI界面...")
        
        # 创建GUI实例
        gui = StockPoolGUI(pool_manager)
        
        # 设置回调函数
        gui.set_stock_selection_callback(lambda code: logger.info(f"选中股票: {code}"))
        gui.set_pool_change_callback(lambda pool_type: logger.info(f"切换到{pool_type}池"))
        
        # 加载初始数据
        gui._show_status("正在加载股票池数据...")
        
        # 这里可以添加数据加载逻辑
        
        gui._show_status("股票池GUI界面就绪")
...
def start_gui():
    """启动GUI模式"""
    try:
        logger.info("🎨 启动股票池管理系统GUI模式")
        
        # 创建股票池管理器
        pool_manager = PoolManager()
        
        # 启动GUI
        launch_gui(pool_manager)
...
def main():
    ...
    try:
        ...
        parser = argparse.ArgumentParser(description='股票池管理系统')
        parser.add_argument('--gui', action='store_true', help='启动GUI界面')
        parser.add_argument('--sync', action='store_true', help='同步股票池数据')
        parser.add_argument('--monitor', action='store_true', help='启动监控模式')
        parser.add_argument('--analyze', action='store_true', help='运行技术分析')
        ...
        if args.gui:
            ...
            # 数据同步模式
            logger.info("🔄 启动数据同步模式...")
            pool_manager = PoolManager(force_refresh_cache=args.force_refresh)
            ...
            # 这里可以添加监控逻辑
            logger.info("✅ 监控模式启动完成")
            return True
            
        elif args.analyze:
            # 技术分析模式
            logger.info("📈 启动技术分析模式...")
            pool_manager = PoolManager()
